...
xray_data['Finding_strings']=xray_data['Finding Labels'].map(lambda x: '1' if 'Consolidation' in x else '0')
#xray_data['Finding_strings']=pd.get_dummies(xray_data['Finding Labels'])

AP = xray_data[(xray_data['Finding Labels']=="Consolidation") & (xray_data['View Position']=="AP")]
PA = xray_data[(xray_data['Finding Labels']=="Consolidation") & (xray_data['View Position']=="PA")]

unaugmented_data = xray_data[(xray_data['Finding Labels']!="Consolidation")]
...
xray_data_test['Finding_strings']=xray_data_test['Finding Labels'].map(lambda x: '1' if 'Consolidation' in x else '0')
...
xray_data_valid['Finding_strings']=xray_data_valid['Finding Labels'].map(lambda x: '1' if 'Consolidation' in x else '0')
...
out_file = open("Consolidation.yaml","w")

out_file.write(model_yaml)
out_file.close()

model.save_weights('Consolidation_1.h5')
...
g.figure.savefig('../charts and graphs/trainingloss for Consolidation.png')

h = sns.lineplot(x= test_loss, y=epoch_count)
h.figure.save('../charts and graphs/testingloss for Consolidation.png')

i = sns.lineplot(x= accuracy, y=epoch_count)
i.figure.save('../charts and graphs/accuracy for Consolidation.png')

j = sns.lineplot(x= testing_accuracy, y=epoch_count)
j.figure.save('../charts and graphs/validation accuracy for Consolidation.png')
